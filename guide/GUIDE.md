...
answers the command line, less the runtime's own options (`--threads`, `--gpu`,
`--gpu-build`, `--bend-help`; a `--` ends them). A handle (`File`, `Socket`,
`Window`) is an affine, opaque value, so every effect on one hands it back beside
its result, and no program can forge or reuse one.
